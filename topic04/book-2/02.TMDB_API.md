...
+ The statement: `const { id } = props.match.params` allows the component to extract the movie id from the browser parameterized URL address. 
...
+ The component retrieves the full list of genres from TMDB and stores them in a local state.
